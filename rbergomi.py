...
# code adapted from:
# https://github.com/ryanmccrickerd/rough_bergomi/blob/master/rbergomi/rbergomi.py
...
class rBergomi(object):
    ...
    def __init__(self, n=100, N=1000, T=1.00, r=0.01, a=-0.4):
        """
    ...
    def S(self, V, dB, S0=1):
        ...
        increments = np.sqrt(V[:, :-1]) * dB + (self.r - 0.5 * V[:, :-1]) * dt
    ...
    def S1(self, V, dW1, rho, S0=1):
        ...
        # Construct non-anticipative Riemann increments
        increments = rho * np.sqrt(V[:, :-1]) * dW1[:, :, 0] + (self.r - 0.5 * rho ** 2 * V[:, :-1]) * dt
